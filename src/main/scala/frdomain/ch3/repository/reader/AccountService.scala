...
object AccountService extends AccountService[Account, Amount, Balance] {
  ...
  def open(no: String, name: String, openingDate: Option[Date]): Reader[AccountRepository, Try[Account]] = Reader {
  ...
  def close(no: String, closeDate: Option[Date]): Reader[AccountRepository, Try[Account]] = Reader {
    (repo: AccountRepository) =>
      repo.query(no) match {
        case Success(Some(a)) =>
  ...
  def debit(no: String, amount: Amount): Reader[AccountRepository, Try[Account]] = Reader { (repo: AccountRepository) =>
    repo.query(no) match {
      case Success(Some(a)) =>
  ...
  def credit(no: String, amount: Amount): Reader[AccountRepository, Try[Account]] = Reader {
    (repo: AccountRepository) =>
      repo.query(no) match {
