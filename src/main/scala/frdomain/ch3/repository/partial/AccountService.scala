...
object AccountService extends AccountService[Account, Amount, Balance] {
  ...
  def close(no: String, closeDate: Option[Date]): AccountRepository => Try[Account] = (repo: AccountRepository) =>
    repo.query(no) match {
      case Success(Some(a)) =>
  ...
  def debit(no: String, amount: Amount): AccountRepository => Try[Account] = (repo: AccountRepository) =>
    repo.query(no) match {
      case Success(Some(a)) =>
  ...
  def credit(no: String, amount: Amount): AccountRepository => Try[Account] = (repo: AccountRepository) =>
    repo.query(no) match {
